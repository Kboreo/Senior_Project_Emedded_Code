...
void timer23Init(void)
{
  ...
  PR2 = 0xC400-1;     //Period for least significant word of (time) 0.25s timer
...
void takeDown(void)
{    
    STATUS_LED_SetHigh(void);   //Turn status LED on to let user know the microcontroller is currently "Working".
    timer23Init(void);  //Initialize 32 bit 2/3 Timer
    retractMotor(backRight);
    STATUS_LED_SetLow();    //Turn status LED off to let user know the microcontroller is not currently "Working".
    disableAllMotors(void);
}
/**
 End of File
*/